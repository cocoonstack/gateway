...
|------|----------|-----------|------|
| `openai` | `https://api.openai.com` | openai-chat, embeddings, image, tts, stt, responses, completions, realtime, moderations, video | `Bearer` (realtime live-verified on gpt-realtime-mini — the bridge settles the vendor's `response.done` usage, audio output at its `token_rate.audio_completion` weight, `estimated: false`; video = Sora, see below; GPT-6 Astra live-verified on chat + Responses: automatic prompt caching bills `prompt_tokens_details.cache_write_tokens` at `token_rate.write_cache` on the first call and `cached_tokens` after, and the model takes `low`–`xhigh` on chat completions, `low`–`max` on Responses, never `none` or `minimal`; GPT-5.6 luna/sol/terra live-verified too — same cache fields, `none`–`xhigh` on chat, `max` from 5.6 on Responses, and client sampling knobs only at effort `none` or with no effort; GPT-6 Sol and Luna live-verified — same cache fields, `none`–`xhigh` on chat and `none`–`max` on Responses and Bedrock, reasoning by default, sampling knobs only at `none`; chat completions refuse function tools while a 5.6 or 6 model reasons, so a tool-calling client sends `reasoning_effort: none` there or the model is served as `protocol: responses`, which takes tools at any effort and is Astra's only tool route) |
...
|----------|--------|----------|-------|
| `gemini` | Google Gemini | `https://generativelanguage.googleapis.com` | `x-goog-api-key`; streams via `streamGenerateContent`; thinking tokens billed as reasoning; the native wire carries text and inline images only — serve tool use through Google's OpenAI-compatible endpoint (`kind: openai`, `endpoint: …/v1beta/openai`) |
...
else from its preset `kind` — `openai` → Sora,
