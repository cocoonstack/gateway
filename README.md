...
- **OpenAI + Anthropic compatible surface** — `/v1/chat/completions`, `/v1/completions`, `/v1/responses`, `/v1/messages`, `/v1/embeddings`, `/v1/images/{generations,edits}`, `/v1/videos/generations` + `/v1/videos/{id}`, `/v1/audio/{speech,transcriptions,translations}`, `/v1/moderations`, `/v1/rerank`, `/v1/batches` + `/v1/files`, `/v1/models`, `/v1/realtime` (WebSocket) — streaming and non-streaming
...
- **Fast** — the whole pipeline (auth, admission, DLP, engine, billing) costs ~24 µs per request in-process; over HTTP one node serves ~90k requests/s at p99 under 10 ms on small bodies and ~40k/s at 256 concurrency on 52 KB / 13k-token prompts, mock upstream ([numbers and method](docs/performance.md))
