...
mod tests {
    ...
    async fn cost_budgets_deny_at_the_cap_and_alert() {
        ...
        let ev = alerts.try_recv().expect("the key cap raises an alert");
        assert_eq!(ev.kind, "budget_exhausted");
        assert!(ev.subject.starts_with("key:sha256:"), "{}", ev.subject);
        assert!(
            ev.detail.starts_with("daily cost budget: "),
            "{}",
            ev.detail
        );
        assert!(
            alerts.try_recv().is_err(),
            "the tenant pool is under its cap"
        );
        let err = h
            .run(req("second is over"), key)
            .await
            .err()
            .expect("second denied by the key cost budget");
