...
#[tokio::test]
async fn a_redacted_native_responses_stream_that_fails_ends_with_an_error_event() {
    #[derive(Debug)]
    struct FailingPiiStream;
    #[async_trait::async_trait]
    impl gw_engines::transport::Transport for FailingPiiStream {
        async fn send(
            &self,
            _req: gw_engines::transport::UpstreamRequest,
        ) -> gw_models::GResult<gw_engines::transport::UpstreamResponse> {
            ...
            let frames = [
                Ok::<_, gw_engines::transport::StreamFault>(bytes::Bytes::from(
                    "data: {\"type\":\"response.output_text.delta\",\"delta\":\"write to leak@evil.com\"}\n\n",
                )),
                Ok(bytes::Bytes::from(
                    "data: {\"type\":\"response.failed\",\"response\":{\"status\":\"failed\",\"error\":{\"code\":\"server_error\",\"message\":\"boom\"},\"usage\":{\"input_tokens\":3,\"output_tokens\":5}}}\n\n",
                )),
            ];
            Ok(gw_engines::transport::UpstreamResponse {
                status: 200,
...
"#;
    let cfg = Arc::new(GatewayConfig::from_yaml(yaml).unwrap());
    let state = Arc::new(GatewayState::from_config(&cfg));
    let app = gw_views::app(AppState::new(cfg, state, Arc::new(FailingPiiStream)));
...
async fn a_tool_call_cut_by_max_tokens_finishes_with_length() {
    ...
    assert_eq!(
        ...
        "truncated tool arguments must not read as a finished call"
    );
    assert_eq!(v["choices"][0]["message"]["tool_calls"][0]["id"], "tool-1");
}
